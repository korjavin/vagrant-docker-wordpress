...
config.vm.define "wmysql1" do |v|
  v.vm.provider "docker" do |mysql|
       mysql.name    = "wmysql1"
       mysql.image   = "korjavin/docker-mysql-port"
       mysql.has_ssh = false
       mysql.volumes = ["/media/Space/wordpress-mysql1:/var/lib/mysql"]
  end
end
config.vm.define "wordpress1" do |v|
  v.vm.provider "docker" do |wordpress|
    wordpress.name  = "wordpress1"
    wordpress.image = "library/wordpress"
    wordpress.ports = ["80:80"]
    wordpress.link("wmysql1:mysql")
